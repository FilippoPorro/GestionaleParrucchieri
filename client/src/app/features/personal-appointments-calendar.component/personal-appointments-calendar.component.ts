...
@Component({
  selector: 'app-personal-appointments-calendar',
  standalone: true,
  imports: [CommonModule, FormsModule, FullCalendarModule],
  templateUrl: './personal-appointments-calendar.component.html',
  styleUrls: [
    './personal-appointments-calendar.component.css',
    '../../features/appuntamenti.component/appuntamenti.component.css'
  ],
})
export class PersonalAppointmentsCalendarComponent implements OnInit, AfterViewInit, OnDestroy {
  @Input({ required: true }) operatorId!: number;
  @Input() operatorName = '';
  @Input() operatorEmail = '';
  ...
  private readonly calendarViewStorageKey = 'personal_appointments_calendar_view';
  ...
  private readonly initialCalendarDate = new Date();
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      next: ({ operatori, clienti }) => {
        this.operatori = this.resolvePersonalOperatorList(operatori);
        this.clienti = clienti;
        this.selectedOperator = this.operatori[0]?.idUtente ?? null;
  ...
  selectOperator(idOperatore: number | null): void {
    ...
    if (this.selectedOperator === idOperatore || idOperatore !== this.operatorId) {
      return;
    }
  ...
  private resolvePersonalOperatorList(operators: Utente[]): Utente[] {
    const operator = operators.find((item) => item.idUtente === this.operatorId);

    if (operator) {
      return [operator];
    }

    const nameParts = this.operatorName.trim().split(/\s+/).filter(Boolean);
    const nome = nameParts.shift() || 'Operatore';

    return [{
      idUtente: this.operatorId,
      nome,
      cognome: nameParts.join(' '),
